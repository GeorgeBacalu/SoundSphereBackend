...
namespace SoundSphere.Tests.Integration.Repositories
{
    public class ArtistRepositoryIntegrationTest : IClassFixture<DbFixture>
    {
        ...
        private void Execute(Action<ArtistRepository, SoundSphereContext> action)
        {
        ...
        [Fact] public void FindById_InvalidId_Test() => Execute((artistRepository, context) => 
            artistRepository.Invoking(repository => repository.FindById(Constants.InvalidGuid))
        ...
        [Fact] public void Save_Test() => Execute((artistRepository, context) =>
        {
            ...
            context.Artists.Find(newArtist.Id).Should().BeEquivalentTo(newArtist);
        });
        ...
        [Fact] public void UpdateById_InvalidId_Test() => Execute((artistRepository, context) =>
            artistRepository.Invoking(repository => repository.UpdateById(_artist2, Constants.InvalidGuid))
                            .Should().Throw<ResourceNotFoundException>()
                            .WithMessage($"Artist with id {Constants.InvalidGuid} not found!"));
        ...
        [Fact] public void DisableById_InvalidId_Test() => Execute((artistRepository, context) =>
            artistRepository.Invoking(repository => repository.DisableById(Constants.InvalidGuid))
                            .Should().Throw<ResourceNotFoundException>()
                            .WithMessage($"Artist with id {Constants.InvalidGuid} not found!"));
